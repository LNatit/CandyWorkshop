...
public interface StatRegistry {
    DeferredRegister<Identifier> STATS =
            DeferredRegister.create(Registries.CUSTOM_STAT, CandyWorkshop.MODID);

    DeferredHolder<Identifier, Identifier> INTERACT_WITH_SUGAR_REFINERY =
            STATS.register(
                    "interact_with_sugar_refinery",
                    () -> CandyWorkshop.id("interact_with_sugar_refinery")
            );
    DeferredHolder<Identifier, Identifier> OPEN_DRAWER_TABLE =
            STATS.register(
                    "open_drawer_table",
